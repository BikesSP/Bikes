...
namespace ClientService.Application.UserPost.Handler
{
    public class CancelApplicationHandler: IRequestHandler<CancelApplicationRequest, Response<bool>>
    {
        ...
        public CancelApplicationHandler(
            ILogger<CancelApplicationHandler> logger, IUnitOfWork unitOfWork, ICurrentUserService currentUserService)
        {
            _logger = logger;
        ...
        public async Task<Response<bool>> Handle(CancelApplicationRequest request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                var cancelApplierQuery = await _unitOfWork.AccountRepository.GetAsync(x => x.Id.ToString() == user.Id.ToString());
                var cancelApplier = cancelApplierQuery.FirstOrDefault();
